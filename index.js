...
const sqlite3 = require('sqlite3').verbose(); // Make sure sqlite3 is required
...
const commandsDir = path.join(__dirname, 'commands'); // New: commands directory

if (!fs.existsSync(publicDir)) fs.mkdirSync(publicDir);
if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir);
if (!fs.existsSync(commandsDir)) fs.mkdirSync(commandsDir); // New: Create commands directory
...
app.post('/webhook', async (req, res) => {
    try {
        console.log('📩 Received webhook event:', JSON.stringify(req.body, null, 2)); // Log full body for debugging
                    ...
                    try {
                        if (messageText.startsWith(PREFIX)) {
                            ...
                            if (command) {
                                ...
                                const commandResponse = await command.execute(senderId, args, bot.pageAccessToken);
                                await sendFacebookMessage(senderId, commandResponse, bot.pageAccessToken);
                                await storeMessage(senderId, commandResponse, 'bot', 'text', { command: commandName });
                            } else {
